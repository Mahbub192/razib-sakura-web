        ...
        <nav className="flex flex-col gap-2 mt-4">
          {navigationItems.map((item) => {
            ...
            return (
              ...
              >
                <span className={`material-symbols-outlined text-2xl ${active ? 'fill' : ''}`}>{item.icon}</span>
                <p className={`text-sm leading-normal ${active ? 'font-semibold' : 'font-medium'}`}>{item.label}</p>
              </Link>
            )
          })}
        </nav>
